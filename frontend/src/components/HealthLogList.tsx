...
export function HealthLogList({ logs }: { logs: HealthCheckLog[] }) {
  if (logs.length === 0) {
    return <p className="py-4 font-mono text-sm text-text-muted">No check history yet.</p>;
  }
  ...
  return (
    <ul className="flex flex-col">
      {logs.map((log) => (
        <li
          key={log.id}
          className="flex items-start gap-4 border-b border-border py-2 last:border-b-0"
        >
